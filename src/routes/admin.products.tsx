...
const CATS = ["medicine", "supplements", "cosmetics"] as const;

type Row = {
  id: string; slug: string; image_url: string | null; category: string; position: number;
...
function AdminProductsPage() {
  ...
  const load = async () => {
    ...
    const { data } = await supabase.from("products").select("*").order("category").order("position");
  ...
  const create = async () => {
    const { supabase } = await import("@/integrations/supabase/client");
    const { data, error } = await supabase.from("products").insert({ slug: `prod-${Date.now()}`, name_ru: "Новый продукт", category: "medicine" }).select("*").single();
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <h1 className="text-display text-2xl font-extrabold">{t("admin.menu.products")}</h1>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[320px_1fr]">
        <div className="space-y-2">
          {items.map((n) => (
            <button key={n.id} onClick={() => setEditing(n)}
              ...
              <div>
                <p className="text-sm font-semibold">{n.name_ru || n.slug}</p>
        ...
        {editing && (
          <div className="rounded-xl border border-border bg-card p-6 space-y-4">
            ...
            <Inp label="Image URL" value={editing.image_url ?? ""} onChange={(v) => setEditing({ ...editing, image_url: v })} />
            <div>
              <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-muted-foreground">Категория</label>
              <select value={editing.category} onChange={(e) => setEditing({ ...editing, category: e.target.value })} className="w-full rounded-md border border-input bg-background px-4 py-2.5 text-sm">
